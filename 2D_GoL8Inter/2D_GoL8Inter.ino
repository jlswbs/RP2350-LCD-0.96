// 8 x 2D Game of Life cellular automata - center and interlayer rules //
  ...
  for (int layer = 0; layer < NUM_LAYERS; layer++) {
    ...
    for (int i = -half_size; i <= half_size; i++) {
      for (int j = -half_size; j <= half_size; j++) {
        set_cell(grid, mid_x + i, mid_y + j, layer, true);
      }
    }
  }
}
  ...
  for (int i = -1; i <= 1; i++) {
    for (int j = -1; j <= 1; j++) {
      for (int k = -1; k <= 1; k++) {
        ...
        int nl = (layer + k + NUM_LAYERS) % NUM_LAYERS;
        count += get_cell(grid, nx, ny, nl);
      }
    }
  }
  return count;
