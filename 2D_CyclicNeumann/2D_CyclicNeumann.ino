// 2D Cyclic cellular automata - Von Neumann rule //
...
void update_automaton() {
  ...
  for (int y = 0; y < HEIGHT; y++) {
    ...
    for (int x = 0; x < WIDTH; x++) {
      ...
      for (int i = 0; i < 4; i++) {
        int nx = (x + dirs[i][0] + WIDTH) % WIDTH;
        int ny = (y + dirs[i][1] + HEIGHT) % HEIGHT;
        uint8_t neighbor_state = grid[IDX(nx, ny)];
        
        if (neighbor_state == ((current + 1) % N_STATES)) next_state_neighbors++;
      }
      
      if (next_state_neighbors >= 1) next_state = (current + 1) % N_STATES;
