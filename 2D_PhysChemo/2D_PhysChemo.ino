...
float senseTrail(int species, float x, float y) {
  int ix = (int)x;
  int iy = (int)y;
  if (ix < 1 || ix >= WIDTH-1 || iy < 1 || iy >= HEIGHT-1) return 0.0f;
...
void diffuseTrail(int species) {
  for (int y = 1; y < HEIGHT-1; y++) {
    for (int x = 1; x < WIDTH-1; x++) {
      ...
      sum += trail[species][x-1][y] + trail[species][x+1][y];
      sum += trail[species][x][y-1] + trail[species][x][y+1];
      sum += trail[species][x-1][y-1] + trail[species][x+1][y-1];
      sum += trail[species][x-1][y+1] + trail[species][x+1][y+1];
...
void loop(){
  ...
  for (int y = 0; y < HEIGHT; y++){
    for (int x = 0; x < WIDTH; x++){
      ...
      for (int i = 0; i < TOTAL_AGENTS; i++) {
        int ax = (int)agents[i].x;
        int ay = (int)agents[i].y;
        if (ax == x && ay == y) {
